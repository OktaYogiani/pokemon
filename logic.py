...
class Fighter(Pokemon):
    async def attack(self, enemy):
        Fighter_super = random.randint(5,15)
        self.power += Fighter_super
        hasil = await super().attack(enemy)
        self.power -= Fighter_super
        return hasil + f"\nPetarung menggunakan serangan super dengan kekuatan:{Fighter_super}"
    async def feed(self, feed_interval = 10, hp_increase = 20):
        current_time = datetime.now()  
        delta_time = timedelta(mintues=feed_interval)  
        if (current_time - self.last_feed_time) > delta_time:
            self.hp += hp_increase
            self.last_feed_time = current_time
            return f"Kesehatan Pokemon dipulihkan. HP saat ini: {self.hp}"
